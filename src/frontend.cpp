...
{
	auto compareKey = [](screeninfo const & x)
	{
		return std::make_tuple(x.screen, x.width, x.height);
	};
	auto compareLess = [&](screeninfo const & a, screeninfo const & b)
	{
		return compareKey(a) < compareKey(b);
	};
	auto compareEq = [&](screeninfo const & a, screeninfo const & b)
	{
		return compareKey(a) == compareKey(b);
	};
